...
from hddcoin.cmds.cmds_util import get_wallet
...
async def app(hddcoin_config, hddcoin_root, secure_the_bag_targets_path: str, leaf_width: int, tail_hash_bytes: bytes32, unwind_target_puzzle_hash_bytes: bytes32, genesis_coin_id: bytes32, fingerprint:int, wallet_id:int, unwind_fee: int):
    full_node_client = await FullNodeRpcClient.create(hddcoin_config["self_hostname"], hddcoin_config["full_node"]["rpc_port"], hddcoin_root, load_config(hddcoin_root, "config.yaml"))
    wallet_client = await WalletRpcClient.create(hddcoin_config["self_hostname"], hddcoin_config["wallet"]["rpc_port"], hddcoin_root, load_config(hddcoin_root, "config.yaml"))
...
def cli(
    ...
    hddcoin_root: Path = Path(os.path.expanduser(os.getenv("CHIA_ROOT", "~/.hddcoin/mainnet"))).resolve()
    hddcoin_config = load_config(hddcoin_root, "config.yaml")

    asyncio.get_event_loop().run_until_complete(
        app(hddcoin_config, hddcoin_root, secure_the_bag_targets_path, leaf_width, tail_hash_bytes, unwind_target_puzzle_hash_bytes, eve_coin_id, fingerprint, wallet_id, unwind_fee)
    )
